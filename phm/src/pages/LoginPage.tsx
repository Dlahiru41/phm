...
export const LoginPage: React.FC = () => {
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        if (user) {
            ...
            setError(TranslationService.t('login.invalidCredentials'));
            setPassword('');
        }
    };
    ...
    const handleSendOTP = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            if (!forgotEmail.trim()) {
                setForgotPasswordError(TranslationService.t('forgotPassword.invalidEmail'));
            ...
            if (res) {
                setForgotPasswordSuccess(TranslationService.t('forgotPassword.otpSentToMobile'));
                setForgotPasswordStep('otp');
            }
        } catch (err: any) {
            setForgotPasswordError(err?.message || TranslationService.t('forgotPassword.failed'));
        } finally {
            setIsLoadingForgotPassword(false);
        }
    };
    ...
    const handleVerifyOTPAndReset = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            if (!otpCode.trim()) {
                setForgotPasswordError(TranslationService.t('forgotPassword.invalidOTP'));
                setIsLoadingForgotPassword(false);
                return;
            }

            if (!newPassword) {
                setForgotPasswordError(TranslationService.t('forgotPassword.passwordTooShort'));
                setIsLoadingForgotPassword(false);
                return;
            }

            if (newPassword !== confirmPassword) {
                setForgotPasswordError(TranslationService.t('forgotPassword.passwordMismatch'));
                setIsLoadingForgotPassword(false);
                return;
            }
            ...
            if (res) {
                setForgotPasswordSuccess(TranslationService.t('forgotPassword.passwordResetSuccess'));
                setTimeout(() => {
                    closeForgotPassword();
                    setForgotEmail('');
                    setOtpCode('');
            ...
            <div className="hidden lg:flex lg:w-1/2 relative bg-primary items-center justify-center overflow-hidden">
                ...
                <div className="relative z-20 px-20 text-white">
                    ...
                    <p className="text-lg font-normal leading-relaxed text-white/90 max-w-md">
                        {TranslationService.t('login.hero.description')}
                    </p>
                    <div className="mt-12 flex items-center gap-4">
                        <div className="h-1 w-12 bg-white rounded-full"></div>
                        <p className="text-sm font-bold tracking-wider uppercase">{TranslationService.t('login.hero.footer')}</p>
                    </div>
                </div>
            </div>
                ...
                <div className="w-full max-w-[480px]">
                    ...
                    <div className="mb-10 px-4">
                        <div className="flex items-center justify-between mb-6">
                            ...
                            {/* Language Selector - Only available on Login Page */}
                            <div className="flex gap-2">
                                {TranslationService.getAllLanguages().map((lang) => (
                                    <button
                                        key={lang.code}
                        ...
                        <h2 className="text-[#0d141b] dark:text-white text-3xl font-bold leading-tight tracking-[-0.015em]">{TranslationService.t('login.title')}</h2>
                        <p className="text-[#4c739a] dark:text-slate-400 mt-2">{TranslationService.t('login.subtitle')}</p>
                    </div>
                    ...
                    <form className="flex flex-col gap-2" onSubmit={handleSubmit}>
                        <div className="flex flex-wrap items-end gap-4 px-4 py-3">
                            <label className="flex flex-col min-w-40 flex-1">
                                <p className="text-[#0d141b] dark:text-white text-base font-medium leading-normal pb-2">{TranslationService.t('login.nicOrEmail')}</p>
                                <input
                                    ...
                                    placeholder={TranslationService.t('login.nicOrEmail')}
                                    type="text"
                                    value={username}
                        ...
                        <div className="flex flex-wrap items-end gap-4 px-4 py-3">
                            <label className="flex flex-col min-w-40 flex-1">
                                <p className="text-[#0d141b] dark:text-white text-base font-medium leading-normal pb-2">{TranslationService.t('login.password')}</p>
                                <div className="flex w-full flex-1 items-stretch rounded-lg">
                                    <input
                                        ...
                                        placeholder={TranslationService.t('login.enterPassword')}
                        ...
                        <div className="flex justify-between items-center px-4 mb-4">
                            ...
                            >
                                {TranslationService.t('login.forgotPassword')}
                            </p>
                        </div>
                        ...
                        <div className="px-4 py-3">
                            <button
                                ...
                                type="submit"
                            >
                                <span className="truncate">{TranslationService.t('login.signIn')}</span>
                            </button>
                        </div>
                    </form>
                    ...
                    <div className="mt-8 px-4 text-center">
                        <div className="relative py-4">
                            ...
                            <div className="relative flex justify-center text-sm">
                                <span className="bg-background-light dark:bg-background-dark px-2 text-[#4c739a]">{TranslationService.t('login.newToMohPortal')}</span>
                            </div>
                        </div>
                        <p className="text-[#4c739a] dark:text-slate-400 text-sm mt-4">
                            {TranslationService.t('login.dontHaveAccount')}
                        </p>
                        <div className="mt-4">
                            <Link
                                to="/register"
                                ...
                                {TranslationService.t('login.createAccount')}
                            </Link>
                        </div>
                    </div>
            ...
            {showForgotPassword && (
                <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
                    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl max-w-md w-full p-8">
                        <div className="flex items-center justify-between mb-6">
                            <h2 className="text-2xl font-bold text-slate-900 dark:text-white">{TranslationService.t('forgotPassword.title')}</h2>
                            <button
                                onClick={closeForgotPassword}
                                className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300"
                            >
                                <span className="material-symbols-outlined">{TranslationService.t('common.close')}</span>
                            </button>
                        </div>
                            ...
                            <form onSubmit={handleSendOTP} className="space-y-4">
                                <div>
                                    <p className="text-sm text-slate-600 dark:text-slate-400 mb-4">
                                        {TranslationService.t('forgotPassword.emailHint')}
                                    </p>
                                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                                        {TranslationService.t('forgotPassword.enterEmail')}
                                    </label>
                                    <input
                                        type="email"
                                        value={forgotEmail}
                                        onChange={(e) => setForgotEmail(e.target.value)}
                                        placeholder={TranslationService.t('forgotPassword.enterEmail')}
                                ...
                                >
                                    {isLoadingForgotPassword ? TranslationService.t('common.loading') : TranslationService.t('forgotPassword.sendOTP')}
                                </button>
                            </form>
                        )}
                        ...
                        {(forgotPasswordStep === 'otp' || forgotPasswordStep === 'reset') && (
                            <form onSubmit={handleVerifyOTPAndReset} className="space-y-4">
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                                        {TranslationService.t('forgotPassword.enterOTP')}
                                    </label>
                                    <input
                                        type="text"
                                        value={otpCode}
                                        onChange={(e) => setOtpCode(e.target.value.replace(/\D/g, '').slice(0, 6))}
                                        placeholder={TranslationService.t('forgotPassword.otpHint')}
                                        maxLength={6}
                                ...
                                {forgotPasswordStep === 'reset' && (
                                    <>
                                        <div>
                                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                                                {TranslationService.t('forgotPassword.enterNewPassword')}
                                            </label>
                                            <div className="relative">
                                                <input
                                                    ...
                                                    placeholder={TranslationService.t('forgotPassword.enterNewPassword')}
                                        ...
                                        <div>
                                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                                                {TranslationService.t('forgotPassword.confirmPassword')}
                                            </label>
                                            <div className="relative">
                                                <input
                                                    ...
                                                    placeholder={TranslationService.t('forgotPassword.confirmPassword')}
                                                    className="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary pr-10"
                                                    required
                                                />
                                                <button
                                ...
                                <div className="flex gap-3">
                                    ...
                                    >
                                        {TranslationService.t('forgotPassword.back')}
                                    </button>
                                    <button
                                        type="submit"
                                        disabled={isLoadingForgotPassword}
                                    ...
                                    >
                                        {isLoadingForgotPassword ? TranslationService.t('common.loading') : forgotPasswordStep === 'reset' ? TranslationService.t('forgotPassword.resetPassword') : TranslationService.t('forgotPassword.verifyOTP')}
                                    </button>
                                </div>
                            </form>
                        )}
